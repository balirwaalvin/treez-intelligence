...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  const saveToHistory = async (newMessages: ChatMessage[]) => {
  ...
  const handleSendMessage = async (text: string = input) => {
    ...
    // Don't save yet, wait for model response to save pair? Or save user msg first?
    // Saving pair is better for "sessions", but saving user msg prevents loss.
    // Let's save conservatively after user message for now if it's the valid flow
      ...
      setMessages((prev) =>
        prev.map((msg) =>
          msg.isStreaming
            ? {
                ...msg,
                text: error.message || "Error generating image",
                isStreaming: false,
              }
            : msg,
        ),
      ...
      <div className="relative z-10 flex flex-col items-center animate-fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 w-full max-w-2xl mb-10">
          {[
            ...
            {
              icon: Code,
              text: "Debug React code",
              desc: "Find & fix errors",
              color: "from-cyan-500/10 to-blue-500/10",
              iconColor: "text-cyan-400 group-hover:text-cyan-300",
            },
            {
              icon: TrendingUp,
              text: "Brainstorm strategies",
            ...
            >
              <div className="flex items-start gap-3">
                <div
                  className={`p-2.5 rounded-xl bg-gradient-to-br ${chip.color} transition-all duration-300`}
                >
                  <chip.icon
                    size={18}
                    className={`transition-colors ${chip.iconColor}`}
                  />
                </div>
                <div>
        ...
        <div className="w-full max-w-2xl relative z-20">
          <div className="bg-[#0a0a16] rounded-2xl p-2 border border-white/[0.06] focus-within:border-treez-indigo/30 focus-within:shadow-[0_0_40px_rgba(99,102,241,0.12)] transition-all duration-300 flex items-center gap-2">
            ...
            <button
              ...
              disabled={
                (!input.trim() && !attachment) || isStreaming || !canSend
              }
          ...
          <div className="mt-3 flex items-center justify-center gap-3 text-xs text-gray-600">
            <span className="flex items-center gap-1.5">
              <div
                className={`w-1.5 h-1.5 rounded-full ${user ? "bg-emerald-500 shadow-[0_0_6px_rgba(16,185,129,0.5)]" : "bg-amber-500 shadow-[0_0_6px_rgba(245,158,11,0.3)]"}`}
              />
              {user
                ? user.plan === "pro"
                  ? "Treez Pro"
            ...
            {!user && (
              <>
                ...
                <span>
                  {usageService.getRemainingPrompts()} prompts remaining
                </span>
              </>
            )}
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 scroll-smooth">
        {messages.map((msg) => (
          ...
          >
            ...
            <div
              className={`flex flex-col gap-2 max-w-[80%] ${msg.role === MessageRole.USER ? "items-end" : "items-start"}`}
            >
              {msg.images &&
                msg.images.map((img, idx) => (
                  <div
                    key={idx}
                    className="rounded-xl overflow-hidden border border-white/10 mb-1 max-w-sm"
                  >
                    <img
                      src={img}
                      alt="User upload"
              ...
              >
                      ...
                      components={{
                        code({
                          ...
                          return !inline && match ? (
                            ...
                            <code
                              className={`${className} bg-white/10 px-1.5 py-0.5 rounded text-treez-accent`}
                              {...props}
                            >
                              {children}
                            </code>
                        ...
                        h1: ({ children }) => (
                          <h1 className="text-xl font-bold mb-3 mt-4 first:mt-0 text-white border-b border-white/10 pb-2">
                            {children}
                          </h1>
                        ),
                        h2: ({ children }) => (
                          <h2 className="text-lg font-bold mb-3 mt-4 text-gray-100">
                            {children}
                          </h2>
                        ),
                        h3: ({ children }) => (
                          <h3 className="text-base font-bold mb-2 mt-3 text-gray-200">
                            {children}
                          </h3>
                        ),
                        a: ({ href, children }) => (
                          <a
                            ...
                            className="text-treez-accent hover:underline decoration-treez-accent/50"
                          >
                            {children}
                          </a>
                        ),
                        blockquote: ({ children }) => (
                          <blockquote className="border-l-2 border-treez-accent pl-4 my-3 italic text-gray-400 bg-white/5 py-2 pr-2 rounded-r">
                            {children}
                          </blockquote>
                        ),
                        table: ({ children }) => (
                        ...
                        td: ({ children }) => (
                          <td className="px-4 py-3 text-sm text-gray-300 whitespace-pre-wrap">
                            {children}
                          </td>
                        ),
                      }}
                ...
                {msg.isStreaming && (
                  <span className="inline-block w-1.5 h-4 ml-1 bg-treez-accent align-middle animate-pulse"></span>
                )}
              </div>
      ...
      <div className="p-4 bg-[#050511]/90 backdrop-blur-xl border-t border-white/[0.04] z-20">
        ...
        <div className="text-center mt-2">
          <p className="text-[10px] text-gray-600">
            {user
              ? "End-to-end Encryption · Private Session"
              : `${usageService.getRemainingPrompts()} free prompts remaining`}
          </p>
        </div>
      </div>
    </>
