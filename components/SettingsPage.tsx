...
const SettingsItem = ({
  ...
  <div className="flex items-center justify-between p-4 hover:bg-white/[0.02] transition-colors group">
    <div className="flex items-center gap-4">
      <div
        className={`p-2.5 rounded-xl ${danger ? "bg-red-500/10 text-red-500" : "bg-treez-indigo/10 text-treez-indigo"} group-hover:scale-105 transition-transform duration-300`}
      >
        <Icon size={18} />
      </div>
      <div>
        <div
          className={`font-medium text-sm ${danger ? "text-red-400" : "text-gray-200"}`}
        >
          {label}
        </div>
        {desc && (
          <div className="text-[11px] text-gray-500 mt-0.5 max-w-[200px] leading-tight">
            {desc}
          </div>
        )}
      </div>
...
const Toggle = ({
  ...
  >
    <div
      className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full shadow-md transition-transform duration-200 ${checked ? "translate-x-5" : "translate-x-0"}`}
    />
  </button>
);
...
export const SettingsPage: React.FC<SettingsPageProps> = ({ onBack }) => {
  ...
  const handleClearHistory = async () => {
    if (
      ...
      if (user) {
        await chatService.clearUserChatHistory(user.uid);
      ...
      <div className="max-w-3xl mx-auto w-full p-6 pb-24 space-y-6">
        ...
        <div className="mb-8 bg-gradient-to-br from-treez-indigo/10 to-treez-purple/10 border border-treez-indigo/20 rounded-2xl p-6 flex flex-col md:flex-row items-center gap-6 animate-fade-in relative overflow-hidden group">
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <p className="text-gray-400 text-sm mb-3">
              {user?.email || "Sign in to sync your history"}
            </p>
            <div
              className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border text-xs font-medium ${user?.plan === "pro" ? "bg-treez-accent/10 border-treez-accent/30 text-treez-accent" : "bg-gray-700/30 border-gray-600 text-gray-300"}`}
            >
              {user?.plan === "pro" && (
          ...
          {!user && (
            <button
              onClick={() => {
                /* Trigger auth modal from app level? Need callback */
              }}
        ...
        <SettingsSection title="General">
          ...
          <SettingsItem
            ...
            action={
              <Toggle checked={streamResponses} onChange={setStreamResponses} />
            }
          />
          <SettingsItem
            icon={Globe}
        ...
        <SettingsSection title="Data & Privacy">
          <SettingsItem
            ...
            action={
              <button className="text-xs font-medium text-treez-indigo hover:text-white transition-colors px-3 py-1.5 rounded-lg hover:bg-white/[0.05]">
                Export JSON
              </button>
            }
          />
