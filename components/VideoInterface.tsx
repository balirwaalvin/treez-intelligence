...
import { Video, Loader2, AlertCircle, Zap } from "lucide-react";
import { useAuth } from "../contexts/AuthContext";

export const VideoInterface: React.FC = () => {
  const { user, incrementVideoUsage } = useAuth();
  ...
  const generateVideo = async () => {
    ...
    // Check usage limits
    if (user.plan === "pro") {
      const used = user.videoPromptsUsed || 0;
      if (used >= 15) {
        setError(
          "You have reached your monthly limit of 15 video prompts. Upgrade to Premium Plus for unlimited access.",
        );
        return;
      }
    }
  ...
  const getUsageText = () => {
    ...
    if (user.plan === "pro_plus")
      return (
        <span className="text-treez-accent flex items-center gap-1">
          <Zap size={12} /> Unlimited Access
        </span>
      );
    if (user.plan === "pro")
      return (
        <span className="text-gray-400">
          ...
          <span className="text-white font-medium">
            {user.videoPromptsUsed || 0} / 15
          </span>
        </span>
      );
    return null; // Standard users shouldn't be here, but just in case
  };
      ...
      <div className="max-w-4xl mx-auto w-full space-y-8">
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-gray-400">
                Cinematic video generation powered by Veo.
              </p>
            </div>
            {user && (
        ...
        <div className="space-y-4">
          ...
          >
            {isGenerating ? (
              <>
                <Loader2 className="animate-spin" /> Generating Dream (this
                takes a moment)...
              </>
            ) : (
              <>
        ...
        <div className="p-4 bg-white/5 rounded-lg border border-white/5 text-sm text-gray-400">
          <p className="flex items-center gap-2">
            <AlertCircle size={16} />
            TREEZ Motion uses the Veo model. Videos are generated securely on
            our servers.
          </p>
        </div>
      </div>
